...
int main(void)
{
  ...
  uint32_t threshold = 108000;
  while (1)
    {
    /* USER CODE END WHILE */

    /* USER CODE BEGIN 3 */
  	uint32_t tmp_val = TIM2->CNT;
  	if((tmp_val > threshold) && (check_flag == false))
  	{
  		check_flag = true;
  		HAL_ADC_Start_DMA(&hadc1, (uint32_t*) CCDPixelBuffer, CCDBuffer);
  	}
  	else if((tmp_val <= threshold) && (check_flag == true))
  	{
  		check_flag = false;
  	}
    }

...
output_csv_path = export_path + f"LED_optimization_results_{timestamp}ET.csv"
...
def run_optimization_with_cv(n_combinations):
    ...
    """
    from sklearn.ensemble import ExtraTreesClassifier  # ExtraTreesClassifierをインポート
    ...
    def objective(trial):
        ...
        # ExtraTreesClassifierを使用し、10-foldクロスバリデーションを実施
        model = ExtraTreesClassifier(
            n_estimators=trial.suggest_int("n_estimators", 100, 200),  # 決定木の数
            max_depth=trial.suggest_int("max_depth", 15, 25),  # 木の深さ
            min_samples_split=trial.suggest_int("min_samples_split", 2, 4),  # 分割時の最小サンプル数
            min_samples_leaf=trial.suggest_int("min_samples_leaf", 1, 3),  # 葉に必要な最小サンプル数
            random_state=42
        )
...
# 組み合わせ数を増やしながら最適化
for n in range(1, 11):  # 組み合わせ数を8から10まで

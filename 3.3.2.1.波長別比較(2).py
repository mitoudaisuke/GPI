...
# 結果を保存するCSVのパス
results_csv_path = os.path.join(export_path, "wavelength_pair_comparison_results.csv")
trials_history_csv_path = os.path.join(export_path, "wavelength_pair_comparison_optimization_history.csv")
...
# 波長ペアの処理を行う関数
def process_wavelength_pair(wl_pair, df, classifier_hyperparams):
    wl1, wl2 = wl_pair
    print(f"=== Evaluating wavelengths: {wl1} nm & {wl2} nm ===")

    try:
        # 波長 `wl1` と `wl2` のデータのみを使用
        ...
        # 特徴量とターゲットに分割
        X_df = merged_data.drop(['species', 'turbidity', f'wavelength_wl{wl1}', f'wavelength_wl{wl2}'], axis=1)
        ...
        for split in range(N_OUTER_SPLITS):
            ...
            X_train_val, X_test, y_train_val, y_test = train_test_split(
                X_df, y, test_size=0.15, random_state=RANDOM_STATE + split, stratify=y)
        ...
        print(f"Error processing wavelengths {wl_pair}: {str(e)}")
        return pd.DataFrame()  # 空のDataFrameを返す

def main():
    # データの読み込み
    df = pd.read_csv(import_path, header=0, index_col=0, encoding="sjis")
    wavelengths = np.arange(200, 600, 2)  # 20ずつ飛ばして波長を指定

    # 2つの波長の組み合わせを生成
    wavelength_combinations = list(itertools.combinations(wavelengths, 2))
    print(f"Total wavelength combinations to evaluate: {len(wavelength_combinations)}")

    # 既存の結果を読み込み、既に処理済みの波長ペアを特定
    if os.path.exists(results_csv_path):
        existing_results = pd.read_csv(results_csv_path)
        processed_pairs = set(zip(existing_results['Wavelength_1'], existing_results['Wavelength_2']))
        print(f"Already processed wavelength pairs: {len(processed_pairs)}")
    else:
        existing_results = pd.DataFrame()
        processed_pairs = set()

    # 未処理の波長ペアを特定
    remaining_pairs = [pair for pair in wavelength_combinations if pair not in processed_pairs]
    print(f"Remaining wavelength pairs to process: {len(remaining_pairs)}")

    if not remaining_pairs:
        print("All wavelength pairs have been processed.")
        return
    ...
    # 結果を即時に保存するために、CSVファイルをロックして書き込む
    # ここではシンプルにメインプロセスが書き込むため、ワーカーは結果を返すだけにします

    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        # 各波長ペアを並列で処理
        future_to_pair = {executor.submit(process_wavelength_pair, pair, df, classifier_hyperparams): pair for pair in remaining_pairs}
        ...
        for future in as_completed(future_to_pair):
            ...
            try:
                ...
                if not results_df.empty:
                    ...
                    print(f"Saved results for wavelengths {pair[0]} nm & {pair[1]} nm")
            except Exception as e:
                print(f"Error in processing pair {pair}: {str(e)}")
                continue

    print(f"\nAll wavelength pair comparison results have been saved to {results_csv_path}")

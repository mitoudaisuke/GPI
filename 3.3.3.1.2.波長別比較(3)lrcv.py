...
# ログの設定
logging.basicConfig(
    filename=os.path.join(export_path, 'optimization_log_logisticregressioncv.log'),
    level=logging.INFO,
...
# グローバル設定
RANDOM_STATE = 42
N_CV_SPLITS = 5  # クロスバリデーションの分割数
DESIRED_TOTAL_TRIALS = 40000  # トータルで実行したいトライアル数

# LogisticRegressionCV のハイパーパラメータ最適化設定
classifier_hyperparams = {
    'LogisticRegressionCV': {
        ...
        'params': {
            'Cs': {'type': 'int', 'low': 1, 'high': 10},  # 正則化パラメータCの候補数
            'max_iter': {'type': 'int', 'low': 100, 'high': 500},
            'solver': {'type': 'fixed', 'value': 'lbfgs'},  # 固定
            'cv': {'type': 'fixed', 'value': N_CV_SPLITS}  # 固定
        }
    }
}
...
def save_trial_callback(study, trial):
    ...
    trial_result = {
        ...
        'Cs': trial.params.get('Cs'),
        'max_iter': trial.params.get('max_iter'),
        'solver': trial.params.get('solver'),
        'cv': trial.params.get('cv'),
        'pca_n_components': trial.params.get('pca_n_components'),
        'state': trial.state.name
    }
    trials_csv_path = os.path.join(export_path, "optuna_trials_logisticregressioncv_three_wavelengths.csv")
...
def objective(trial, df, wavelengths, classifier_hyperparams):
    ...
    # ハイパーパラメータをサンプリング
    params = {}
    for param_name, param_info in classifier_hyperparams['LogisticRegressionCV']['params'].items():
    ...
    # PCAの成分数をサンプリング
    pca_config = classifier_hyperparams['LogisticRegressionCV']['pca_n_components']
    ...
    # LogisticRegressionCVの初期化
    model = LogisticRegressionCV(**params, random_state=RANDOM_STATE)
...
def main():
    ...
    storage_path = os.path.join(export_path, "optuna_study_logisticregressioncv.db")
    storage_url = f"sqlite:///{storage_path}"
    sampler = optuna.samplers.TPESampler(n_startup_trials=20000, seed=RANDOM_STATE)  # 最初の10トライアルはランダム
    study = optuna.create_study(
        study_name="wavelength_triplet_optimization_logisticregressioncv",
        storage=storage_url,
        sampler=sampler,
        direction='maximize',
        load_if_exists=True
    ...
    remaining_trials = classifier_hyperparams['LogisticRegressionCV']['n_trials'] - existing_trials

    if remaining_trials <= 0:
        print(f"Study already has {existing_trials} trials, which meets or exceeds the desired {classifier_hyperparams['LogisticRegressionCV']['n_trials']} trials.")
        logging.info(f"Study already has {existing_trials} trials, which meets or exceeds the desired {classifier_hyperparams['LogisticRegressionCV']['n_trials']} trials.")
    else:
        ...
        best_params_csv_path = os.path.join(export_path, "best_params_logisticregressioncv_three_wavelengths.csv")

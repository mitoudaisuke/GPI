...
# ログの設定
logging.basicConfig(
    filename=os.path.join(export_path, 'optimization_log_logisticregression.log'),
    level=logging.INFO,
...
DESIRED_TOTAL_TRIALS = 500  # トータルで実行したいトライアル数
...
def save_trial_callback(study, trial):
    ...
    trials_csv_path = os.path.join(export_path, f"optuna_trials_logisticregression_{num_wavelengths}_wavelengths.csv")
...
def objective(trial, df, wavelengths, classifier_hyperparams, num_wavelengths):
    ...
    # C のサンプリング
    params['C'] = trial.suggest_loguniform('C', classifier_hyperparams['LogisticRegressionCV']['params']['C']['low'],
                                          classifier_hyperparams['LogisticRegressionCV']['params']['C']['high'])
...
def main():
    ...
    for num_wavelengths in wavelength_counts:
        ...
        storage_path = os.path.join(export_path, f"optuna_study_logisticregression_{num_wavelengths}_wavelengths.db")
        ...
        study = optuna.create_study(
            study_name=f"wavelength_{num_wavelengths}_optimization_logisticregression",
            storage=storage_url,
            sampler=sampler,
            direction='maximize',
            load_if_exists=True
        ...
        if remaining_trials <= 0:
            ...
            best_params_csv_path = os.path.join(export_path, f"best_params_logisticregression_{num_wavelengths}_wavelengths.csv")

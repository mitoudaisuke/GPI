...
def prepare_eem_data(df, species, target_turbidity=None):
    ...
    if filtered_df.empty:
        raise ValueError(f"No data found for species '{species}' with turbidity '{target_turbidity}'")

    # ユニークな濁度値を取得（この場合は1つの値になるはず）
...
def main():
    try:
        # ターゲットとする濁度を指定
        target_turbidity = 0.10

        # データの準備
        all_data, emission, excitation, turbidities = prepare_eem_data(df, species, target_turbidity=target_turbidity)
        ...
        plt.savefig(f"{export_path}excitation_collinearity_turbidity_{target_turbidity}.tiff", 
                    dpi=400, bbox_inches='tight', format='tiff')
        plt.close(fig1)
        
        fig2 = plot_collinearity(avg_emission_correlation, 'Emission', emission)
        plt.savefig(f"{export_path}emission_collinearity_turbidity_{target_turbidity}.tiff", 
                    dpi=400, bbox_inches='tight', format='tiff')
        plt.close(fig2)
        
        print(f"濁度 {target_turbidity} の多重共線性ヒートマップの保存が完了しました。")

...
DESIRED_TOTAL_TRIALS = 1000  # トータルで実行したいトライアル数
...
classifier_hyperparams = {
    'MLPClassifier': {
        ...
        'params': {
            'hidden_layer_sizes': {'type': 'int', 'low': 50, 'high': 200},  # 隠れ層のニューロン数
            'activation': {'type': 'fixed', 'value': 'relu'},  # 活性化関数
            'solver': {'type': 'fixed', 'value': 'adam'},  # 最適化手法
            'alpha': {'type': 'loguniform', 'low': 1e-5, 'high': 1e-3},  # L2正則化項
            'learning_rate_init': {'type': 'loguniform', 'low': 1e-4, 'high': 1e-2},  # 学習率
            'max_iter': {'type': 'fixed', 'value': 200},  # 最大イテレーション
        }
    }
}
...
def main():
    ...
    sampler = optuna.samplers.TPESampler(n_startup_trials=50, seed=RANDOM_STATE)  # 最初の10トライアルはランダム
